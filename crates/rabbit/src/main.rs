...
fn main() -> std::process::ExitCode {
    ...
    #[cfg(feature = "gui")]
    {
        if std::env::args_os().count() <= 1 {
            rabbit_ui_wxdragon::run_gui();
            return std::process::ExitCode::SUCCESS;
        }
    }

    attach_parent_console_on_windows();
    match rabbit_cli::run() {
...
/// pop-up on double-click). When the user runs RABBIT from `cmd.exe` /
...
fn attach_parent_console_on_windows() {
    ...
    unsafe {
        ...
        // redirected stdout/stderr (`rabbit.exe ... | tee out.txt`,
        // `rabbit.exe ... > log`), the OS already inherits the pipe/file
