...
pub fn verify_planned_execution_freshness(
    ...
    if stale.is_empty() {
        ...
        Err(RabbitError::PostInstallVerificationFailed {
            missing_paths: stale,
        })
    }
}
...
fn execute_program_plan(plan: &PlannedExecutionPlan) -> Result<()> {
    ...
    let status = command.status().map_err(|source| RabbitError::Io {
    ...
    if !status.success() {
        ...
        // dismissed). The installer never actually ran, so RABBIT surfaces
        ...
        if cfg!(target_os = "windows") && status.code() == Some(1223) {
            return Err(RabbitError::UserCancelledElevation {
                program: program.clone(),
            });
        }
        return Err(RabbitError::ProcessFailed {
            program: program.clone(),
...
fn execute_program_plan_elevated(plan: &PlannedExecutionPlan, program: &str) -> Result<()> {
    ...
    .map_err(|error| match error {
        ...
        other => RabbitError::InvalidPlannedExecution {
    ...
    match exit_code {
        Some(0) => Ok(()),
        Some(code) => Err(RabbitError::ProcessFailed {
            program: program.to_string(),
            exit_code: Some(code),
        }),
        None => Err(RabbitError::ProcessFailed {
            program: program.to_string(),
            exit_code: None,
        }),
    }
...
fn verify_paths(paths: &[PathBuf]) -> Result<()> {
    ...
    if missing_paths.is_empty() {
        ...
        Err(RabbitError::PostInstallVerificationFailed { missing_paths })
    }
}
